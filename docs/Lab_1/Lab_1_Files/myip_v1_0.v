...
// those which are assigned in an always block of myip_v1_0 shoud be changes to reg.
    ...
    localparam NUMBER_OF_INPUT_WORDS  = (1<<A_depth_bits) + (1<<B_depth_bits); // 8+4=12


	// Total number of output data
	localparam NUMBER_OF_OUTPUT_WORDS = (1<<RES_depth_bits);   
        ...
        // ------------------------------------------
			...
			case (state)
                ...
                begin
                    ...
                    if (S_AXIS_TVALID == 1)
                    begin
                        if (read_counter < (1<<A_depth_bits)) begin
                            A_write_en      <= 1;
                            A_write_address <= read_counter;
                            A_write_data_in <= S_AXIS_TDATA[7:0];
                        end else begin
                            B_write_en      <= 1;
                            B_write_address <= read_counter - (1<<A_depth_bits);
